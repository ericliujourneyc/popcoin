...
// popcoin: get network Gh/s estimate
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid popcoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid popcoin address:")+s.name_);
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // popcoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; popcoin server stopping, restart to run with encrypted wallet";
}
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Popcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Popcoin is downloading blocks...");
...
// popcoin: get information of sync-checkpoint
...
// popcoin: reserve balance from being staked for network protection
...
// popcoin: check wallet integrity
...
// popcoin: make a public-private key pair
