...
// Copyright (c) 2011-2015 The Popcoin developers
...
int nCoinbaseMaturity = COINBASE_MATURITY_POP;
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // popcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // popcoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // popcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// popcoin: find block wanted by given orphan block
...
// popcoin: miner's coin stake is rewarded based on coin age spent (coin-days)
int64 GetProofOfStakeReward(int64 nCoinAge)
{
    static int64 nRewardCoinYear = 1.5 * CENT;  // creation amount per coin-year
// COINAGE
...
// popcoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // popcoin: target change every block
    // popcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // popcoin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // popcoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // popcoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // popcoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // popcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // popcoin: track money supply and mint amount info
    ...
    // popcoin: fees are not collected by miners as in bitcoin
    // popcoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// popcoin: total coin age spent in transaction, in the unit of coin-days.
...
// popcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // popcoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // popcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // popcoin: record proof-of-stake hash value
    ...
    // popcoin: compute stake modifier
...
bool CBlock::CheckBlock() const
{
    ...
    // popcoin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // popcoin: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // popcoin: check that the block satisfies synchronized checkpoint
    ...
    // popcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // popcoin: verify hash target and signature of coinstake tx
    ...
    // popcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // popcoin: getblocks may not obtain the ancestor block rejected
    ...
    // popcoin: if responsible for sync-checkpoint send it
...
// popcoin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Popcoin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMinAge = 60 * 30; // test net min age is 30 minute
        nCoinbaseMaturity = 60;
        //nCoinbaseMaturity = 5;
        bnInitialHashTarget = CBigNum(~uint256(0) >> 22);
        //nModifierInterval = 60 * 20; // test net modifier interval is 20 minutes
        nModifierInterval = 10; // test net modifier interval is 10 second
    }
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Popcoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Dr. Luke to Be Dropped By Sony Amid Kesha Legal Battle Mar 10 2016";
        CTransaction txNew;
        txNew.nTime = 1457576920;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1457576920;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1474053166;

        if (fTestNet)
        {
            block.nTime    = 1457576920;
            block.nNonce   = 123446969;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x924607dd5ee46a37517eb8b6b4d6a169510a65faf40edd360c15ce4347ba7703"));
        block.print();
        ...
        // popcoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");

        // popcoin: upgrade time set to zero if txdb initialized
        {
            CTxDB txdb;
    ...
    // popcoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // popcoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // popcoin: should not enter safe mode for longer invalid chain
    // popcoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // popcoin: if detected invalid checkpoint enter safe mode
    ...
    // popcoin: if detected unmet upgrade requirement enter safe mode
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // popcoin: record my external IP reported by peer
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // popcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // popcoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // popcoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // popcoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // popcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
