...
{
    // Wait for IRC to get it first - disabled with popcoin
...
// testnet dns seed begins with 't', all else are popcoin dns seeds.
static const char *strDNSSeed[][2] = {
    {"seed", "seed.popcoin.org"},
//    {"seedppc", "seedppc.popcoin.net"},
//    {"7server", "ppcseed.ns.7server.net"},
//    {"altcointech", "dnsseed.ppc.altcointech.net"},
//    {"diandianbi", "seed.diandianbi.org"},
//    {"tnseed", "tnseed.popcoin.net"},
//    {"tnseedppc", "tnseedppc.popcoin.net"},
};
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // popcoin enables dns seeding with testnet too
    {
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  Popcoin is probably already running."), ntohs(sockaddr.sin_port));
        else
...
void StartNode(void* parg)
{
    ...
    // popcoin: mint proof-of-stake blocks in the background
