...
// popcoin: optional setting to unlock wallet for block minting only;
...
// popcoin: total coins staked (non-spendable until maturity)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                if (pcoin->nTime > nSpendTime)
                    continue;  // popcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // popcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // popcoin: not avatar mode
                    {
...
// popcoin: create coin stake transaction
...
// popcoin: check 'spent' consistency between wallet and txindex
// popcoin: fix wallet spent state according to txindex
...
// popcoin: disable transaction (only for coinstake)
